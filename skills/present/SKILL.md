---
name: present
description: Turn any topic — a codebase, a design, an argument, a concept, a process, a pile of notes — into a single self-contained, interactive HTML page: chapters with a side nav, Mermaid diagrams, UI wireframes, callouts, data-model and api-endpoint blocks for schema/API shapes, annotated code and diffs, and open questions the reader can answer inline. Use when the user says "present X", "make a presentation" / "make a visual doc", "explain X visually", "give me a tour of Y", "teach me X as a page", "turn these notes/this design into a page", "/present", or otherwise wants to SEE and explore a topic instead of reading a wall of chat text — covers non-code topics (an argument, a concept, a process) as well as codebases. Also use when the user pastes a blob starting `<!-- presentation-feedback v1 -->`: that's exported review feedback from a page you rendered earlier, not new prose — resolve it against `references/feedback.md`. For an implementation plan that needs sign-off use present-plan; for a git-diff recap use present-recap. Everything renders fully offline from file:// — no SaaS, no account, no server, no network at view time.
---
...
- **Chapters + side nav for long docs.** `<!-- chapter: Title -->` directives
  ...
  in-page navigation; skip it for a short page.
...
any anchored element (a heading, a paragraph, a step, a diff hunk, a whole
block) marked either **for the agent** or **note to self** (self-notes stay in
the page but never leave it); each open `questions` entry renders as a form —
**Accept default** or **Answer differently**; and a **Review panel** collects
everything plus one overall verdict. **Export** copies a structured blob to
the clipboard. Tell the reader this after you open the page: pin notes, answer
questions, pick a verdict if one applies, then paste the export back into
chat.

If a message **begins with** `<!-- presentation-feedback v1 -->`, it's
exported review feedback, not new prose — process it as data. The full
contract (grammar + ingestion algorithm + a worked example) is
...
and works with the network off. Any notes, answers, or verdict the reader pins
